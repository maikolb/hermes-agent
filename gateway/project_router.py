...
class ProjectRouter:
    ...
    def _migrate(self) -> None:
        ...
        CREATE INDEX IF NOT EXISTS idx_acl_profile_user
            ON acl_entries(profile, user_id);
    ...
    def topic_grant_at_use(self, user_id: object, chat_id: object) -> str | None:
        """ACL verdict for creating/binding Topics OUTSIDE the management Topic.

        Adversarial review of the first cut (28/08) showed deny could
        never win at turn time (denied contexts raise before any grant
        logic runs) and a per-turn lookup blocked the event loop — so
        authorization moved HERE, evaluated at the moment of use, inside
        the router the creator already opens. Three verdicts so the deny
        veto also binds the per-profile config grant:

        - "deny": explicit deny for this user in the CURRENT chat (or a
          blank sender id) — vetoes EVERY non-management grant;
        - "acl": an allow/admin entry anywhere in this profile;
        - None: no ACL opinion (the config grant may still apply).
        """
        ...
        with self._lock:
            if chat:
                denied = self._connection.execute(
                    "SELECT 1 FROM acl_entries WHERE profile=? AND chat_id=? "
                    "AND user_id=? AND effect='deny' LIMIT 1",
                    (self.profile, chat, uid),
                ).fetchone()
                if denied:
                    return "deny"
            row = self._connection.execute(
        ...
        return "acl" if row is not None else None
