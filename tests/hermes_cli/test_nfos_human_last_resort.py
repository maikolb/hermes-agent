...
def test_block_is_recorded_by_its_event_and_a_crash_before_recording_is_adopted_by_target(board, monkeypatch):
    with kb.connect_closing() as conn:
        monkeypatch.setattr(delivery, "_destination_reachable", _reachable(False))
        ...
        def block_then_raise(conn, task_id, **kw):
            assert real_block(conn, task_id, **kw)
            raise RuntimeError("database is locked")

        monkeypatch.setattr(kb, "block_task", block_then_raise)
        assert delivery.sweep_destination_waits(conn) == [(task.id, "wait")]
        monkeypatch.setattr(kb, "block_task", real_block)
        assert _state(conn, task.id)["destination_wait"]["block_event_id"]

        other = _card(conn, 45, spec=_spec(_http_probe()))
        _park(conn, other, monkeypatch)
        real_own = delivery._own_block_event

        def crash(conn, task_id, after_id, reason):
            raise RuntimeError("gateway stopped")

        monkeypatch.setattr(delivery, "_own_block_event", crash)
        delivery.sweep_destination_waits(conn)
        monkeypatch.setattr(delivery, "_own_block_event", real_own)
        assert kb.get_task(conn, other.id).status == "blocked"
        wait = _state(conn, other.id)["destination_wait"]
        assert wait["target"] == TARGET and wait.get("block_event_id") is None
        _shift(conn, other.id, next_check_at=0)
        delivery.sweep_destination_waits(conn)
        assert _state(conn, other.id)["destination_wait"]["block_event_id"]
        monkeypatch.setattr(delivery, "_destination_reachable", _reachable(True))
        _shift(conn, other.id, next_check_at=0)
        delivery.sweep_destination_waits(conn)
        _confirmed_up(conn, other.id)
        assert (other.id, "back") in delivery.sweep_destination_waits(conn)
...
def test_dropped_wait_withdraws_its_open_escalation_but_keeps_one_already_answered(board, monkeypatch):
    with kb.connect_closing() as conn:
        ...
        delivery.sweep_destination_waits(conn)
        row = delivery.get_decision(conn, asked)  # revisão 11: pergunta mandada e sem resposta sai com a espera, senão ficaria sem lembrete
        assert row["status"] == "superseded" and json.loads(row["context"])["superseded_reason"] == "destination_wait_dropped"
        assert "destination_wait" not in _state(conn, other.id)

        third = _card(conn, 76, spec=_spec(_http_probe()))
        answered = _escalate(conn, third, monkeypatch)
        delivery.resolve_decision(conn, answered, action="human", author="Principal", answer="PERGUNTA para Jhonatan: o servidor do TEST foi desligado?")
        ctx = json.loads(delivery.get_decision(conn, answered)["context"])
        ctx["human_reply"] = {"answer": "Religuei.", "source": {"platform": "telegram", "message_id": "4250"}, "author": "Jhonatan"}
        conn.execute("UPDATE nfos_decisions SET context=? WHERE id=?", (json.dumps(ctx), answered))
        conn.commit()
        assert kb.unblock_task(conn, third.id)
        assert kb.block_task(conn, third.id, reason="Janela de manutenção do provedor em andamento", kind="transient")
        delivery.sweep_destination_waits(conn)
        assert delivery.get_decision(conn, answered)["status"] == "human"  # resposta recebida fica para o reconcile aplicar
...
# --- revisão 5 (rejulgamento c93ce3bc) --------------------------------------------------------------------------------------------------

def _expire_destination_check(conn, task_id):
    st = _state(conn, task_id)
    st["destination_check"]["next_check_at"] = 0
    conn.execute("UPDATE nfos_workflows SET state_json=? WHERE task_id=?", (json.dumps(st), task_id))
    conn.commit()
...
def test_released_card_is_not_parked_again_on_the_old_measurement(board, monkeypatch):
    with kb.connect_closing() as conn:
        task = _card(conn, 46, spec=_spec(_http_probe()))
        decision_id = _escalate(conn, task, monkeypatch)
        delivery.resolve_decision(conn, decision_id, action="changes", author="Principal",
                                  answer="O TEST mudou para https://novo.example.com; corrija spec, deploy e medição.")
        assert delivery.sweep_destination_waits(conn) == [(task.id, "released")]
        _expire_destination_check(conn, task.id)
        monkeypatch.setattr(delivery, "_destination_reachable", lambda target, timeout=5: pytest.fail("parked again before the worker measured"))
        assert delivery.sweep_destination_waits(conn) == []
        assert delivery.sweep_destination_waits(conn) == []
        assert kb.get_task(conn, task.id).status == "ready" and "destination_wait" not in _state(conn, task.id)

        conn.execute("UPDATE tasks SET status='running', current_run_id=? WHERE id=?", (task.current_run_id, task.id))  # o worker mede de novo
        conn.commit()
        time.sleep(1.1)  # medição nova precisa ser posterior ao evento de liberação (segundos inteiros)
        _park(conn, task, monkeypatch)
        _expire_destination_check(conn, task.id)
        monkeypatch.setattr(delivery, "_destination_reachable", _reachable(False))
        assert delivery.sweep_destination_waits(conn) == [(task.id, "wait")]
...
def test_reconsideration_after_the_measurement_lets_the_worker_act_before_parking(board, monkeypatch):
    with kb.connect_closing() as conn:
        task = _card(conn, 47, spec=_spec(_http_probe()))
        _park(conn, task, monkeypatch)
        with kb.write_txn(conn):
            kb._append_event(conn, task.id, "nfos_principal_reconsidered", {"decision_id": "nd_x", "action": "changes"})
        monkeypatch.setattr(delivery, "_destination_reachable", lambda target, timeout=5: pytest.fail("must not park after a reconsideration"))
        assert delivery.sweep_destination_waits(conn) == []
        assert kb.get_task(conn, task.id).status == "ready"


def test_block_that_does_not_happen_leaves_no_wait_nor_false_next_action(board, monkeypatch):
    with kb.connect_closing() as conn:
        task = _card(conn, 48, spec=_spec(_http_probe()))
        _park(conn, task, monkeypatch)
        monkeypatch.setattr(delivery, "_destination_reachable", _reachable(False))
        before_action = delivery.get_workflow(conn, task.id)["next_action"]
        real_block = kb.block_task

        monkeypatch.setattr(kb, "block_task", lambda conn, task_id, **kw: False)
        assert delivery.sweep_destination_waits(conn) == []
        assert "destination_wait" not in _state(conn, task.id)
        assert delivery.get_workflow(conn, task.id)["next_action"] == before_action

        def claimed_then_block(conn, task_id, **kw):  # o card foi reivindicado entre a espera e o bloqueio
            conn.execute("UPDATE tasks SET status='running' WHERE id=?", (task_id,))
            conn.commit()
            return real_block(conn, task_id, **kw)

        _expire_destination_check(conn, task.id)
        monkeypatch.setattr(kb, "block_task", claimed_then_block)
        assert delivery.sweep_destination_waits(conn) == []
        assert kb.get_task(conn, task.id).status == "running" and "destination_wait" not in _state(conn, task.id)

        other = _card(conn, 49, spec=_spec(_http_probe()))
        _park(conn, other, monkeypatch)

        def foreign_block_first(conn, task_id, **kw):  # outro bloqueio chega antes; max(id) pegaria o dele
            return real_block(conn, task_id, reason="Janela de manutenção do provedor em andamento", kind="transient")

        monkeypatch.setattr(kb, "block_task", foreign_block_first)
        assert (other.id, "wait") not in delivery.sweep_destination_waits(conn)
        monkeypatch.setattr(kb, "block_task", real_block)
        assert kb.get_task(conn, other.id).status == "blocked" and "destination_wait" not in _state(conn, other.id)
        monkeypatch.setattr(delivery, "_destination_reachable", lambda target, timeout=5: pytest.fail("a foreign block is not watched"))
        assert delivery.sweep_destination_waits(conn) == []


def test_destination_block_without_its_wait_gets_the_wait_back(board, monkeypatch):
    with kb.connect_closing() as conn:
        task = _card(conn, 50, spec=_spec(_http_probe()))
        _park(conn, task, monkeypatch)
        monkeypatch.setattr(delivery, "_destination_reachable", _reachable(False))
        assert delivery.sweep_destination_waits(conn) == [(task.id, "wait")]
        block_id = _state(conn, task.id)["destination_wait"]["block_event_id"]
        st = _state(conn, task.id)
        ...
        assert wait["block_event_id"] == block_id and wait["target"] == TARGET and wait["recovered"]
        monkeypatch.setattr(delivery, "_destination_reachable", _reachable(True))
        _shift(conn, task.id, next_check_at=0)
        delivery.sweep_destination_waits(conn)
        _confirmed_up(conn, task.id)
...
def test_answer_attached_to_two_questions_reaches_the_worker_without_being_taken_as_the_operator_answer(board, monkeypatch):
    with kb.connect_closing() as conn:
        ...
        delivery.resolve_decision(conn, asked, action="human", author="Principal",
                                  answer="PERGUNTA para Jhonatan: o servidor do TEST foi desligado?")
        conn.execute("INSERT INTO nfos_decisions(id,task_id,run_id,kind,question,context,spec_revision,created_at) VALUES('dec_plano',?,?,?,?,?,?,?)",
                     (task.id, task.current_run_id, "impediment", "Qual plano Hotmart conta como pago?", "{}", 1, int(time.time()) + 5))
        conn.execute("UPDATE nfos_decisions SET status='human', action='human', author='Principal', answer=? WHERE id='dec_plano'",
                     ("PERGUNTA para Jhonatan: qual plano Hotmart conta como pago?",))
        conn.commit()
...
def test_escalation_of_a_blocked_card_gets_reminders_and_a_visible_stall(board, monkeypatch):
    with kb.connect_closing() as conn:
        task = _card(conn, 66, spec=_spec(_http_probe()))
        asked = _escalate(conn, task, monkeypatch)

        def requested():
            return conn.execute("SELECT count(*) FROM task_events WHERE task_id=? AND kind='nfos_principal_requested'", (task.id,)).fetchone()[0]

        conn.execute("UPDATE nfos_decisions SET created_at=? WHERE id=?", (int(time.time()) - delivery.DECISION_REMINDER_AFTER - 5, asked))
        conn.commit()
        before = requested()
        delivery.sweep_destination_waits(conn)
        assert requested() == before + 1  # lembrete ao Principal mesmo com o card bloqueado

        stale = int(time.time()) - delivery.DECISION_REMINDER_AFTER - delivery.DECISION_REMINDER_GAP * (delivery.DECISION_MAX_REMINDERS + 1)
        ctx = json.loads(delivery.get_decision(conn, asked)["context"])
        ctx["reminders"] = [stale + delivery.DECISION_REMINDER_GAP * i for i in range(delivery.DECISION_MAX_REMINDERS)]
        conn.execute("UPDATE nfos_decisions SET created_at=?, context=? WHERE id=?", (stale, json.dumps(ctx), asked))
        conn.commit()
        delivery.sweep_destination_waits(conn)
        assert delivery.get_workflow(conn, task.id)["next_action"].startswith("Aguardando o Principal")
        assert conn.execute("SELECT count(*) FROM task_events WHERE task_id=? AND kind='nfos_decision_stalled'", (task.id,)).fetchone()[0] == 1
        blocked = kb.get_task(conn, task.id)
        assert blocked.status == "blocked" and blocked.block_kind == "transient" and "destination_wait" in _state(conn, task.id)
...
def test_legacy_link_needs_a_question_newer_than_the_wait(board, monkeypatch):
    with kb.connect_closing() as conn:
        task = _card(conn, 67, spec=_spec(_http_probe()))
        asked = _escalate(conn, task, monkeypatch)
        ...
        st = _state(conn, task.id)
        for key in ("escalation_decision", "escalated_at", "token"):
            st["destination_wait"].pop(key, None)
        st["destination_wait"]["since"] = int(time.time())  # espera legada recente, sem gatilho de escalada nesta varredura
        conn.execute("UPDATE nfos_workflows SET state_json=? WHERE task_id=?", (json.dumps(st), task.id))
        ctx = json.loads(delivery.get_decision(conn, asked)["context"])
        ctx.pop("wait_id", None)
        started = conn.execute("SELECT max(created_at) FROM task_events WHERE task_id=? AND kind='nfos_destination_wait'", (task.id,)).fetchone()[0]
        conn.execute("UPDATE nfos_decisions SET context=?, created_at=? WHERE id=?", (json.dumps(ctx), started - 3600, asked))
        conn.commit()
        reason = delivery.get_workflow(conn, task.id)["next_action"]
        assert delivery.resume_after_answer(conn, task.id, answer="Religuei.", source={"platform": "telegram", "actor": "Jhonatan", "message_id": "4247"})
        assert delivery.get_workflow(conn, task.id)["next_action"] == reason
        assert "destination_wait" in _state(conn, task.id)


def test_stall_note_of_a_long_destination_is_still_recognised(board, monkeypatch):
    with kb.connect_closing() as conn:
        task = _card(conn, 68)
        target = "https://infotributos.example.com/" + "segmento/" * 40
        note = ("Aguardando o Principal há 95 min (3 lembretes sem resposta). Pergunta: Destino " + target + " sem resposta há 400 min")[:400]
        conn.execute("UPDATE nfos_workflows SET next_action=? WHERE task_id=?", (note, task.id))
        conn.commit()
        assert delivery._destination_next_action(conn, task.id, "Destino voltou.", wait={"target": target})
        assert delivery.get_workflow(conn, task.id)["next_action"] == "Destino voltou."


# --- revisão 10 (rejulgamento b5234677) -------------------------------------------------------------------------------------------------

def test_escalation_reminder_never_auto_continues_the_question(board, monkeypatch):
    with kb.connect_closing() as conn:
        task = _card(conn, 69, spec=_spec(_http_probe()))
        _park(conn, task, monkeypatch)
        monkeypatch.setattr(delivery, "_destination_reachable", _reachable(False))
        assert delivery.sweep_destination_waits(conn) == [(task.id, "wait")]
        ...
        delivery.sweep_destination_waits(conn)
        row = delivery.get_decision(conn, asked)
        assert row["status"] == "pending" and json.loads(row["context"]).get("reminders")


def test_question_waiting_for_a_human_gets_a_daily_reminder_and_a_visible_note(board, monkeypatch):
    with kb.connect_closing() as conn:
        task = _card(conn, 70, spec=_spec(_http_probe()))
        asked = _escalate(conn, task, monkeypatch)
        delivery.resolve_decision(conn, asked, action="human", author="Principal", answer="PERGUNTA para Jhonatan: o servidor do TEST foi desligado?")
        ...
        before = requested()
        delivery.sweep_destination_waits(conn)
        assert requested() == before  # pergunta recém-feita: sem lembrete
        conn.execute("UPDATE nfos_decisions SET resolved_at=? WHERE id=?", (int(time.time()) - delivery.DESTINATION_HUMAN_REMINDER_SECONDS - 5, asked))
        conn.commit()
        delivery.sweep_destination_waits(conn)
        assert requested() == before + 1
        assert delivery.get_workflow(conn, task.id)["next_action"].startswith(f"Aguardando resposta humana sobre o destino {TARGET}")
        delivery.sweep_destination_waits(conn)
        assert requested() == before + 1  # um lembrete por período
        blocked = kb.get_task(conn, task.id)
        assert blocked.status == "blocked" and blocked.block_kind == "transient"
        monkeypatch.setattr(delivery, "_destination_reachable", _reachable(True))
        _shift(conn, task.id, next_check_at=0)
...
def test_legacy_link_without_a_wait_anchor_fails_closed(board, monkeypatch):
    with kb.connect_closing() as conn:
        task = _card(conn, 71, spec=_spec(_http_probe()))
        asked = _escalate(conn, task, monkeypatch)
        delivery.resolve_decision(conn, asked, action="human", author="Principal", answer="PERGUNTA para Jhonatan: o servidor do TEST foi desligado?")
        st = _state(conn, task.id)
        for key in ("escalation_decision", "escalated_at", "token"):
            st["destination_wait"].pop(key, None)
        st["destination_wait"]["since"] = int(time.time())
        conn.execute("UPDATE nfos_workflows SET state_json=? WHERE task_id=?", (json.dumps(st), task.id))
        ctx = json.loads(delivery.get_decision(conn, asked)["context"])
        ctx.pop("wait_id", None)
        conn.execute("UPDATE nfos_decisions SET context=? WHERE id=?", (json.dumps(ctx), asked))
        conn.execute("DELETE FROM task_events WHERE task_id=? AND kind='nfos_destination_wait'", (task.id,))
        conn.commit()
        reason = delivery.get_workflow(conn, task.id)["next_action"]
        assert delivery.resume_after_answer(conn, task.id, answer="Religuei.", source={"platform": "telegram", "actor": "Jhonatan", "message_id": "4248"})
        assert delivery.get_workflow(conn, task.id)["next_action"] == reason
        assert "destination_wait" in _state(conn, task.id)


def test_legacy_escalation_answer_is_bounded_by_creation_not_resolution(board, monkeypatch):
    with kb.connect_closing() as conn:
        task = _card(conn, 72, spec=_spec(_http_probe()))
        asked = _escalate(conn, task, monkeypatch)
        delivery.resolve_decision(conn, asked, action="changes", author="Principal", answer="Endereço novo: https://novo.example.com")
        started = conn.execute("SELECT max(created_at) FROM task_events WHERE task_id=? AND kind='nfos_destination_wait'", (task.id,)).fetchone()[0]
        conn.execute("UPDATE task_events SET payload=json_remove(payload, '$.wait_id') WHERE task_id=? AND kind='nfos_destination_wait'", (task.id,))
        ctx = json.loads(delivery.get_decision(conn, asked)["context"])
        ctx.pop("wait_id", None)
        conn.execute("UPDATE nfos_decisions SET context=?, created_at=?, resolved_at=? WHERE id=?", (json.dumps(ctx), started - 3600, started + 5, asked))
        conn.commit()
        assert "Answer to the destination escalation" not in delivery.case_context(conn, task.id)


def test_open_escalation_of_an_older_wait_is_replaced_not_adopted(board, monkeypatch):
    with kb.connect_closing() as conn:
        task = _card(conn, 73, spec=_spec(_http_probe()))
        _park(conn, task, monkeypatch)
        monkeypatch.setattr(delivery, "_destination_reachable", _reachable(False))
        assert delivery.sweep_destination_waits(conn) == [(task.id, "wait")]
        started = conn.execute("SELECT max(created_at) FROM task_events WHERE task_id=? AND kind='nfos_destination_wait'", (task.id,)).fetchone()[0]
        conn.execute("INSERT INTO nfos_decisions(id,task_id,run_id,kind,question,context,spec_revision,created_at) VALUES('dec_velha',?,?,?,?,?,?,?)",
                     (task.id, task.current_run_id, "impediment", "Destino sem resposta?", json.dumps({"destination_wait": True, "target": TARGET}), 1, started - 7200))
        conn.commit()
        _shift(conn, task.id, since=int(time.time()) - delivery.DESTINATION_ESCALATE_SECONDS - 60)
        assert (task.id, "escalated") in delivery.sweep_destination_waits(conn)
        old = delivery.get_decision(conn, "dec_velha")
        assert old["status"] == "superseded" and json.loads(old["context"])["superseded_reason"] == "destination_wait_stale"
        assert _state(conn, task.id)["destination_wait"]["escalation_decision"] != "dec_velha"


def test_initial_reason_of_a_long_destination_is_still_replaced(board, monkeypatch):
    with kb.connect_closing() as conn:
        task = _card(conn, 74)
        target = "https://infotributos.example.com/" + "segmento/" * 45
        reason = ("Destino " + target + " sem resposta desde 14/09 15:02Z (medição AC-01: URLError).")[:400]
        conn.execute("UPDATE nfos_workflows SET next_action=? WHERE task_id=?", (reason, task.id))
        conn.commit()
        assert delivery._destination_next_action(conn, task.id, "Destino voltou.", wait={"target": target})
        assert delivery.get_workflow(conn, task.id)["next_action"] == "Destino voltou."


def test_redelivered_message_already_used_for_another_question_does_not_close_the_escalation(board, monkeypatch):
    with kb.connect_closing() as conn:
        task = _card(conn, 75, spec=_spec(_http_probe()))
        asked = _escalate(conn, task, monkeypatch)
        delivery.resolve_decision(conn, asked, action="human", author="Principal", answer="PERGUNTA para Jhonatan: o servidor do TEST foi desligado?")
        source = {"platform": "telegram", "actor": "Jhonatan", "message_id": "900"}
        with kb.write_txn(conn):  # a mesma mensagem já respondeu outra pergunta antes
            kb._append_event(conn, task.id, "nfos_human_answer_received", {"answer": "O plano anual.", "source": source, "decisions": ["dec_plano"]})
        reason = delivery.get_workflow(conn, task.id)["next_action"]
        assert delivery.resume_after_answer(conn, task.id, answer="O plano anual.", source=source)
        assert delivery.get_workflow(conn, task.id)["next_action"] == reason
        assert "destination_wait" in _state(conn, task.id)
        assert "Answer to the destination escalation" not in delivery.case_context(conn, task.id)


# --- revisão 11 (rejulgamento 944ba7bf) -------------------------------------------------------------------------------------------------

def test_card_back_in_the_queue_withdraws_an_unanswered_human_escalation(board, monkeypatch):
    with kb.connect_closing() as conn:
        task = _card(conn, 77, spec=_spec(_http_probe()))
        asked = _escalate(conn, task, monkeypatch)
        delivery.resolve_decision(conn, asked, action="human", author="Principal", answer="PERGUNTA para Jhonatan: o servidor do TEST foi desligado?")
        assert kb.unblock_task(conn, task.id)
        delivery.sweep_destination_waits(conn)
...
def test_stale_escalation_with_a_received_answer_is_kept_for_the_reconcile(board, monkeypatch):
    with kb.connect_closing() as conn:
        task = _card(conn, 78, spec=_spec(_http_probe()))
        _park(conn, task, monkeypatch)
        monkeypatch.setattr(delivery, "_destination_reachable", _reachable(False))
        assert delivery.sweep_destination_waits(conn) == [(task.id, "wait")]
        started = conn.execute("SELECT max(created_at) FROM task_events WHERE task_id=? AND kind='nfos_destination_wait'", (task.id,)).fetchone()[0]
        ctx = {"destination_wait": True, "target": TARGET, "wait_id": "espera-antiga",
               "human_reply": {"answer": "O IP mudou para 15.229.99.10.", "source": {"platform": "telegram", "message_id": "4251"}, "author": "Jhonatan"}}
        conn.execute("INSERT INTO nfos_decisions(id,task_id,run_id,kind,question,context,spec_revision,created_at) VALUES('dec_respondida',?,?,?,?,?,?,?)",
                     (task.id, task.current_run_id, "impediment", "Destino sem resposta?", json.dumps(ctx), 1, started - 7200))
        conn.execute("UPDATE nfos_decisions SET status='human', action='human', author='Principal', answer='PERGUNTA para Jhonatan: o TEST mudou?' WHERE id='dec_respondida'")
        conn.commit()
        _shift(conn, task.id, since=int(time.time()) - delivery.DESTINATION_ESCALATE_SECONDS - 60)
        assert (task.id, "escalated") not in delivery.sweep_destination_waits(conn)
        assert delivery.get_decision(conn, "dec_respondida")["status"] == "human"


def test_stale_escalation_is_not_withdrawn_while_another_decision_is_open(board, monkeypatch):
    with kb.connect_closing() as conn:
        task = _card(conn, 79, spec=_spec(_http_probe()))
        _park(conn, task, monkeypatch)
        monkeypatch.setattr(delivery, "_destination_reachable", _reachable(False))
        assert delivery.sweep_destination_waits(conn) == [(task.id, "wait")]
        started = conn.execute("SELECT max(created_at) FROM task_events WHERE task_id=? AND kind='nfos_destination_wait'", (task.id,)).fetchone()[0]
...
def test_human_phase_reminder_counts_from_the_question_not_from_older_reminders(board, monkeypatch):
    with kb.connect_closing() as conn:
        ...
        asked = _escalate(conn, task, monkeypatch)
        ctx = json.loads(delivery.get_decision(conn, asked)["context"])
        ctx["reminders"] = [int(time.time()) - delivery.DESTINATION_HUMAN_REMINDER_SECONDS - 3600]  # lembrete antigo da fase pending
        conn.execute("UPDATE nfos_decisions SET context=? WHERE id=?", (json.dumps(ctx), asked))
        conn.commit()
        delivery.resolve_decision(conn, asked, action="human", author="Principal", answer="PERGUNTA para Jhonatan: o servidor do TEST foi desligado?")
