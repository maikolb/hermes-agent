"""Reject generated work after attributed prompt context changes during inference."""
...
def test_context_changed_during_decomposition_discards_graph(tmp_path, monkeypatch):
    ...
    with kb.connect_closing() as conn:
        kb.add_comment(conn, task_id, "operator", original_body)
    ...
    def generate_obsolete_graph(**kwargs):
        ...
        with kb.connect_closing() as editor:
            kb.add_comment(editor, task_id, "operator", latest_body)
        payload = {
            "fanout": True,
            "tasks": [
    ...
    with kb.connect_closing() as conn:
        ...
        assert root.body == original_body
        assert kb.list_comments(conn, task_id)[-1].body == latest_body
