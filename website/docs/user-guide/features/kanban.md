...
| `executor_profiles` | `[]` | Profiles this host spawns workers for. When set, a task created or promoted with an assignee outside the list resolves to `default_assignee`, and the dispatcher re-points a ready task assigned elsewhere (a retired profile whose directory survives, `default`, a placeholder) to `default_assignee` on the same tick, recording an `assigned` event with `source: dispatcher.heal` and the previous value. Without `default_assignee` such a task is moved to `blocked` with a comment. Empty = any existing profile directory counts as an executor; tasks assigned to a non-profile lane keep being skipped as `skipped_nonspawnable`. |
