...
# Media generated by tools DURING the current turn (video storyboard
# sheets, frame fan-outs) is protected from pass 3.5 up to this token
# budget — 8 images at the flat estimate, matching vision_analyze's
# maximum storyboard fan-out.  Above the cap the OLDEST current-turn
# media degrades first, keeping the newest frames live (G5, 29/08:
# mid-analysis compaction kept only 3 of 8 sheets and the video
# analysis lost most of its evidence — DOVCRM 28/08 17:17).
_CURRENT_TURN_MEDIA_TOKEN_CAP = 8 * _IMAGE_TOKEN_ESTIMATE


def _count_images_in_tool_content(content: Any) -> int:
    """Number of embedded image parts in a tool-result body (both shapes)."""
    if isinstance(content, dict) and content.get("_multimodal"):
...
def _last_user_index(result: List[Dict[str, Any]]) -> Optional[int]:
    """Index of the most recent user message — the current turn's anchor.

    Everything after it (tool results, assistant steps) belongs to the
    turn being executed right now; a mid-turn compaction must not eat
    the media that turn is actively analyzing.
    """
...
def _retire_stale_tool_result_images(
    ...
    current_turn_start_idx: Optional[int] = None,
) -> int:
    ...
    Tool results at or after ``current_turn_start_idx`` (the current
    turn's media) are protected up to ``_CURRENT_TURN_MEDIA_TOKEN_CAP``
    and never count against ``keep_newest``; above the cap the oldest
    current-turn media degrades first (G5 contract: the cap is in
    TOKENS, never message count — one image can outweigh fifty video
    references).  ``None`` preserves the legacy global behaviour.
    ...
    for i in range(len(result) - 1, -1, -1):
        ...
        if not _tool_content_has_images(msg.get("content")):
            continue
        if current_turn_start_idx is not None and i >= current_turn_start_idx:
            # The cap meters MEDIA tokens only (flat per-image estimate) —
            # the message's text does not count against the media budget.
            est = _count_images_in_tool_content(msg.get("content")) * _IMAGE_TOKEN_ESTIMATE
            if current_kept_tokens + est <= _CURRENT_TURN_MEDIA_TOKEN_CAP:
                current_kept_tokens += est
                continue
            # Over the current-turn budget: the walk is newest-first, so
            # what lands here is the OLDEST media of the turn.
            new_msg = _strip_images_from_tool_msg(msg)
            if new_msg is not None:
                result[i] = new_msg
                pruned += 1
                logger.warning(
                    "compaction: current-turn media over %d-token cap; "
                    "degraded oldest in-turn tool image at idx %d "
                    "(text summary and on-disk media remain recoverable)",
                    _CURRENT_TURN_MEDIA_TOKEN_CAP, i,
                )
            continue
        seen += 1
        if seen <= keep_newest:
...
class ContextCompressor(ContextEngine):
    ...
    def _prune_old_tool_results(
        ...
        # G5 (29/08): media generated DURING the current turn (after the
        # last user message) is protected up to a token cap — a mid-turn
        # compaction used to keep only 3 of a video's 8 storyboard sheets.
        current_turn_idx = _last_user_index(result)
        pruned += _retire_stale_tool_result_images(
            result, current_turn_start_idx=current_turn_idx
        )
        ...
        if protect_tail_tokens is not None and protect_tail_tokens > 0 and result:
            ...
            def _is_current_turn_media(i: int) -> bool:
                return (
                    current_turn_idx is not None
                    and i >= current_turn_idx
            ...
            if demote_end > prune_boundary and _protected_region_tokens() > soft_ceiling:
                ...
                for i in range(max(0, prune_boundary), demote_end):
                    # G5: media the current turn is analyzing demotes LAST —
                    # the floor/last-resort passes below may still reach it
                    # when nothing else frees enough headroom.
                    if _is_current_turn_media(i):
                        continue
                ...
                if _protected_region_tokens() > soft_ceiling:
                    ...
                    for i in range(max(0, prune_boundary), len(result)):
                        ...
                        if result[i].get("role") == "tool":
                            if _is_current_turn_media(i):
                                logger.warning(
                                    "pressure demotion (final step) stripping "
                                    "CURRENT-TURN media at idx %d — nothing "
                                    "else freed enough headroom", i,
                                )
                            if _demote_tool_result_at(i, spare_protected_skills=False):
                                pressure_hits += 1
