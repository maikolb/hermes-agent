...
RECORD_MODE_PROTOCOL += 'A mandatory probe measures the delivery destination host, never a local or isolated copy (loopback and private hosts are refused unless they are the destination or listed in probe_hosts). A probe credential you obtain legitimately (session cookie, token, read-only DSN) goes to the project probe vault with `probe-env --set NAME` (value on standard input, never in arguments, spec, report or chat) and is referenced as "$env:NAME" in probe headers; `probe-env` alone lists the names. An unreachable destination is not a question: end the turn and the runtime returns the card to the queue when the destination answers.\n'  # HUMAN_LAST_RESORT_20260914
