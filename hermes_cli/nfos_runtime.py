...
Derive success from the request; never substitute a goal that fits the available evidence. Describe
component tests, simulations and supporting demonstrations by what they actually establish, without
presenting them as proof of the requested outcome. If that outcome remains unproven, say so and run
the relevant verification autonomously. Do not invent success, change the goal, add bureaucracy or
ask the owner for closure approval. Reporting-only improvements may follow an independently verified
usable delivery; they must not retain it. The central check is requested result versus observed result.
...
original comments, runs, attachments and workspace. Verify and persist a
...
explicitly changes that task's scope, revise and persist the
...
explicitly requested spec revision, use the selected worker model to write a
JSON spec with goal, criteria [{id,text}], steps and delivery_type. Reuse the
existing context; do not launch an automatic Claude TL consultation or planner.
Preserve explicit model/provider pins. Record the actual analysis evidence and
call `save-spec --input spec.json --evidence evidence.json --author worker`.
No implementation before the applicable spec is persisted.
...
After changes, correct the spec with the selected worker model and submit again.
Instruction/spec changes invalidate acceptance. Do not code before its acceptance.
Use `progress --stage implement --next '...'` before implementation. Persist a
progress state JSON containing useful next steps and recovered files as you work.
Run necessary auxiliary and long verification commands through the native tool CLI
...
and records timeout separately from exit status. Use structured streaming output
when supported. A prior call receipt is available in show;
...
the persisted spec, test each criterion's clarity, scope coverage, verification method,
...
A changed type requires a new matching spec, reusing the existing proposal.
