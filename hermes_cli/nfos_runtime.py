...
Before implementation, read the original request and its attachments, the project's
instructions and relevant project memory. Query that memory from the first run,
not only on continuation. The case/lessons excerpt is not the full project context.
If retrieval is empty or unavailable, say so and inspect the project's saved docs,
decisions and original source. Treat recalled hypotheses as hypotheses.
Wait for spec_review and final_review in this card; these are internal Principal
decisions, not questions to the owner. Reuse an accepted unchanged spec on recovery.
For browser, desktop and other visual criteria, capture the real result in its
execution environment. Link the screenshot and the test/readback to each criterion.
Save external readbacks as local artifacts the Principal can inspect. A screenshot
of the input, a green CI badge or a file's existence does not prove the requested
behavior. Use mandatory criteria with artifact evidence when SQL/HTTP/header probes
cannot observe the behavior. Do not invent an unrelated probe to pass the workflow.
Ask final_review after saving the report. Continue/changes means resume this card.
Preserve FAIL when the outcome remains wrong, even when a probe happens to pass.
"""
