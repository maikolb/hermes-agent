...
# HUMAN_LAST_RESORT_20260914: medição não depende de humano. O worker do card grava a credencial da sonda no cofre do projeto
# (probe-env) e a sonda obrigatória mede o host do destino, nunca uma cópia local ou isolada.
_PROBE_ENV_NAME_RX = re.compile(r'^[A-Z][A-Z0-9_]{0,63}$')


def _probe_vault_file(conn, task_id):
    """HUMAN_LAST_RESORT_20260914: cofre de sonda do projeto (HERMES_HOME/secrets/<project_id>/probe.env), gravado pelo `probe-env`."""
    key, cfg = _probe_project(conn, task_id)
    project_id = str((cfg or {}).get('project_id') or key or '').strip()
    if not project_id or '/' in project_id or '\\' in project_id or project_id.startswith('.'):
        return None
    home = os.environ.get('HERMES_HOME') or str(Path.home() / '.hermes')
    return Path(home) / 'secrets' / project_id / 'probe.env'


def _probe_env_sources(conn, task_id):
    """(env, nomes que vieram só do cofre do probe-env). O probe_env do config do projeto vence em nome repetido."""
    env, vault_names = {}, set()
    vault = _probe_vault_file(conn, task_id)
    if vault is not None and vault.is_file():
        env.update(_env_values(vault))
        vault_names = set(env)
    configured = _probe_env_file(conn, task_id)
    if configured:
        values = _env_values(configured)
        env.update(values)
        vault_names -= set(values)
...
def _destination_host(conn, task_id, spec=None):
    try:
        content = spec if isinstance(spec, dict) else json.loads((get_spec(conn, task_id) or {}).get('content') or '{}')
        scope = content.get('delivery_destination')
        return _host_of(scope.get('target')) if isinstance(scope, dict) else ''
    except Exception:
        return ''
...
def probe_env_command(conn, task_id, *, set_name=None, unset_name=None, value=None):
    """HUMAN_LAST_RESORT_20260914: worker do card ou Principal grava a credencial da sonda no cofre do projeto, sem mexer no config.
    O valor nunca volta na saída nem no evento; só nomes. Credencial em spec, relatório, argumento de comando ou chat segue proibida."""
    worker = os.environ.get('HERMES_KANBAN_TASK')
    if worker and worker != task_id:
        raise WorkflowError('probe-env acts only on the card of this worker')
    if not task_id or get_workflow(conn, task_id) is None:
        raise WorkflowError('probe-env needs the NFOS card (--task)')
    path = _probe_vault_file(conn, task_id)
    if path is None:
        raise WorkflowError('probe-env: the card has no project to hold the probe vault')
...
def _execute_probe(conn, task_id, probe):
    ...
    kind = probe['kind']
    env, vault_names = _probe_env_sources(conn, task_id)  # HUMAN_LAST_RESORT_20260914: cofre do projeto (probe-env) e probe_env do config
    try:
        if kind == 'sql':
            dsn_name = 'PROBE_DATABASE_URL' if env.get('PROBE_DATABASE_URL') else 'DATABASE_URL'
            dsn = env.get(dsn_name)
            if not dsn:
...
def _spec_result_criteria_checks(conn, task_id, spec):
    ...
    for crit in spec.get('criteria') or []:
        ...
        if crit.get('mandatory') and _p.get('kind') in ('http', 'header') and _p.get('phase') != 'before':  # HUMAN_LAST_RESORT_20260914
            problem = _local_probe_problem(conn, task_id, _host_of(_p.get('url')), spec)
            if problem:
                raise WorkflowError(f"Criterion {crit.get('id')}: {problem}")
...
def completion_refusal_note(conn, task_id):
    ...
    _errors = ' '.join(str(e.get('error') or '') for e in pending)  # HUMAN_LAST_RESORT_20260914: rota autônoma para cada medição indisponível
    if re.search(r'HTTP 40[13]', _errors):
        lines.append('Superfície autenticada: faça login legítimo (credencial que já existe no cofre ou fluxo normal da aplicação no destino) e grave a '
                     'credencial da sonda com `probe-env --set NOME` (valor pela entrada padrão); nos headers da sonda use "$env:NOME". Não pergunte a '
                     'humano e não grave credencial na spec.')
    if 'probe vault has no' in _errors:
        lines.append('Sonda SQL sem DSN: grave um acesso de leitura que já existe com `probe-env --set PROBE_DATABASE_URL`. A sonda SQL lê PostgreSQL '
                     'e sqlite; SQL Server se mede pela superfície HTTP do destino.')
...
def case_context(conn, task_id):
    ...
    escalation_answer = _destination_escalation_answer(conn, task_id)  # HUMAN_LAST_RESORT revisão 7: resposta integral da escalada de destino
...
def _human_question_text(answer):
    """Revisão 4: a pergunta sem marcadores iniciais de lista, numeração, citação ou ênfase ('- ', '1. ', '> ', '**'). Sem isso,
    '- PERGUNTA para Maikol: credencial da sonda?' escapava da recusa."""
    return _QUESTION_MARKERS_RX.sub('', str(answer or '').strip())
...
def _human_question_part(answer):
    ...
    text = _human_question_text(answer)
    first = text.split('\n', 1)[0]
    if '?' in first:
        return first
    mark = text.find('?')
...
def _human_addressee(answer):
    match = re.match(r'\s*pergunta\s+(?:para|pro|pra|ao|à|a)\s+([^\s:,?]+)', _human_question_text(answer), re.I)
...
def review_maintenance_human_decisions(conn):
    ...
    resposta, aplica a classe automática."""
    ...
    for row in [dict(r) for r in conn.execute("SELECT * FROM nfos_decisions WHERE status='human' ORDER BY created_at,id").fetchall()]:
        ...
        question = ('Revisão de autonomia (HUMAN_LAST_RESORT_20260914): a pergunta a humano deste card trata de sonda, medição ou manutenção '
                    ...
                    'resposta o runtime espera e devolve o card sozinho; o que não dá para medir agora fecha como entrega parcial com '
                    'continuação. Pergunta recusada: ' + refused[:700])
        ...
        with _kb().write_txn(conn, allow_nested=True):
            ...
            if not current or current['status'] != 'human':
                continue
            ctx.update(superseded_by=new_id, superseded_reason='human_last_resort')
...
def _destination_wait_candidate(conn, task_id):
    """(target, criterion, error, since) quando todo critério obrigatório pendente ficou INDETERMINADO por rede na sonda do host do destino."""
    spec = get_spec(conn, task_id)
    if not spec:
        return None
...
def _store_destination_wait(conn, task_id, wait):
    """Revisão 4: grava a espera (ou a remove, com None) sobre o estado atual do card, dentro da transação do chamador quando houver."""
    with _kb().write_txn(conn, allow_nested=True):
...
def _withdraw_escalation(conn, task_id, wait, reason, statuses=('pending', 'human')):
    """Revisão 4: a pergunta de escalada da espera sai quando perde o objeto (destino voltou) ou o dono (espera descartada), na transação do
    chamador. Pergunta já respondida por humano fica para reconcile_human_answers."""
    decision_id = wait.get('escalation_decision') if isinstance(wait, dict) else None
    escalation = get_decision(conn, decision_id) if decision_id else None
    if not escalation or escalation['status'] not in statuses:
        return None
    try:
        ctx = json.loads(escalation['context'] or '{}') or {}
    except Exception:
        ctx = {}
    if not isinstance(ctx, dict) or ctx.get('human_reply'):
        return None
    ctx.update(superseded_reason=reason)
    conn.execute("UPDATE nfos_decisions SET status='superseded',context=? WHERE id=? AND status=?",
                 (_json(ctx), escalation['id'], escalation['status']))
    _event(conn, task_id, None, 'nfos_destination_escalation_withdrawn', {'decision_id': escalation['id'], 'reason': reason})
...
def _escalate_destination_wait(conn, task, wf, wait, now):
    ...
    destino (servidor desligado, endereço novo). Uma vez por espera; nada se já houver decisão aberta de outro assunto. Revisão 4: a decisão e
    o vínculo na espera nascem na mesma transação, e decisão de escalada aberta sem vínculo é adotada, não duplicada. Devolve 'escalated',
    'adopted' ou None."""
    target = wait.get('target')
    since = int(wait.get('since') or now)
    minutes = (now - since) // 60
    question = (f"Destino {target} sem resposta há {minutes} min (desde {time.strftime('%d/%m %H:%MZ', time.gmtime(since))}; "
                f"medição {wait.get('criterion')}). O runtime segue conferindo a cada 10 min e devolve o card quando o destino responder. "
                "Procure quem opera esse destino, no grupo do projeto, e pergunte se o servidor foi desligado e qual é o endereço atual (decide "
                "human com human_to = essa pessoa). Com endereço novo, responda changes com ele: o card volta ao worker para corrigir spec, "
                "deploy e medição. continue mantém a espera.")
    ...
    with _kb().write_txn(conn, allow_nested=True):
        ...
        conn.execute('INSERT INTO nfos_decisions(id,task_id,run_id,kind,question,context,spec_revision,created_at) VALUES(?,?,?,?,?,?,?,?)',
                     (decision_id, task.id, run[0] if run else 0, 'impediment', question,
                      _json({'destination_wait': True, 'target': target, 'wait_id': wait.get('token')}), wf['spec_revision'], now))  # revisão 7
        wait.update(escalated_at=now, escalation_decision=decision_id)
        _store_destination_wait(conn, task.id, wait)
        _event(conn, task.id, None, 'nfos_destination_escalated', {'decision_id': decision_id, 'target': target, 'minutes': minutes})
...
def _orphan_destination_wait(conn, task):
    ...
    for payload, created_at in conn.execute("SELECT payload, created_at FROM task_events WHERE task_id=? AND kind='nfos_destination_wait' AND id<? "
        ...
        if isinstance(info, dict) and info.get('target') == target:
            ...
            wait_id = info.get('wait_id') or None
            if wait_id:
                wait['token'] = wait_id  # revisão 7: a espera reconstruída mantém o id
            started = int(created_at or started)
            break
    consumed = set()
    for (payload,) in conn.execute("SELECT payload FROM task_events WHERE task_id=? AND kind='nfos_destination_wait_released'", (task.id,)).fetchall():
        try:
            consumed.add((json.loads(payload or '{}') or {}).get('decision_id'))
        except Exception:
            continue
    for decision_id, context, created_at in conn.execute("SELECT id, context, created_at FROM nfos_decisions WHERE task_id=? AND "
        ...
        try:
            ...
            continue
        if (isinstance(ctx, dict) and ctx.get('destination_wait') and ctx.get('target') == target and decision_id not in consumed
                and (ctx.get('wait_id') or None) == wait_id):  # revisão 7: só a escalada desta espera, nunca uma já consumida
            wait.update(escalated_at=int(created_at or 0), escalation_decision=decision_id)
            break
    return wait


def _destination_next_action(conn, task_id, text, wait=None):
    """Revisão 6: o next_action do card, que o worker lê no contexto do caso, acompanha a espera. Revisão 7: com wait só troca o next_action
    que ainda é da espera (o motivo 'Destino <alvo> sem resposta' ou o aviso 'Aguardando o Principal' da escalada dela); sem wait grava
    sempre (resposta do Principal ou humana)."""
    with _kb().write_txn(conn, allow_nested=True):
        wf = get_workflow(conn, task_id)
        if not wf:
            return False
        current = str(wf['next_action'] or '')
        escalation = wait.get('escalation_decision') if isinstance(wait, dict) else None
        if wait is not None and not (current.startswith(f"Destino {wait.get('target')} sem resposta")
                                     or (escalation and current.startswith('Aguardando o Principal') and escalation in current)):
            return False
...
def _destination_wait_answered(conn, task_id, reply, decision_id):
    ...
    transação que desbloqueia o card e vai ao next_action; a resposta integral aparece no contexto do caso."""
    wf = get_workflow(conn, task_id)
    try:
        state = json.loads(wf['state_json'] or '{}') or {} if wf else {}
    except Exception:
        return False
    if not isinstance(state, dict) or not isinstance(state.get('destination_wait'), dict):
        return False
    _store_destination_wait(conn, task_id, None)
    _destination_next_action(conn, task_id, f"Resposta de {reply.get('author') or 'Human'} ({decision_id}; íntegra no contexto do caso): "
                                            f"{reply.get('answer') or ''}")
    return True


def _destination_escalation_answer(conn, task_id):
    """Revisão 7: resposta integral da última escalada de destino resolvida com informação para o worker (changes do Principal ou resposta
    humana) desde a espera mais recente, para o endereço novo chegar inteiro mesmo quando não cabe no next_action."""
    started = conn.execute("SELECT max(created_at) FROM task_events WHERE task_id=? AND kind='nfos_destination_wait'", (task_id,)).fetchone()[0]
    rows = conn.execute("SELECT id, context, action, answer, author FROM nfos_decisions WHERE task_id=? AND status='resolved' AND kind='impediment' "
                        "AND resolved_at>=? ORDER BY resolved_at DESC, id DESC LIMIT 20", (task_id, int(started or 0))).fetchall()
    for decision_id, context, action, answer, author in rows:
        try:
            ctx = json.loads(context or '{}') or {}
        except Exception:
            continue
        if isinstance(ctx, dict) and ctx.get('destination_wait') and answer and (action == 'changes' or author != 'Principal'):
            return f"Answer to the destination escalation {decision_id} ({author}, {action}): {str(answer)[:2000]}"
    return ''
...
def sweep_destination_waits(conn):
    ...
    duas respostas seguidas; passada 1 h sem resposta, abre decisão para o Principal procurar quem opera o destino. Revisão 4: bloqueio,
    espera, escalada e desbloqueio mudam juntos ou a espera fica intacta para a próxima varredura; changes do Principal na escalada devolve o
    card ao worker, continue mantém a espera; espera de card que já saiu do bloqueio acaba, com a escalada pendente."""
    changed = []
    now = int(time.time())
    budget = DESTINATION_MAX_CHECKS_PER_SWEEP
    try:
        rows = conn.execute(  # revisão 3: quem está há mais tempo sem checagem vem primeiro; revisão 4: CAST tolera número gravado como texto
            "SELECT t.id FROM tasks t JOIN nfos_workflows w ON w.task_id=t.id WHERE t.status IN ('ready','blocked') "
            "ORDER BY CASE WHEN json_valid(w.state_json) THEN CAST(COALESCE(json_extract(w.state_json,'$.destination_wait.next_check_at'), "
            "json_extract(w.state_json,'$.destination_check.next_check_at'), 0) AS INTEGER) ELSE 0 END, t.id").fetchall()
    ...
    for (task_id,) in rows:
        try:
            ...
            if task.status == 'blocked':
                ...
                if wait.get('block_event_id') is None and last_id is not None and task.block_kind == 'transient':
                    try:  # espera da revisão 1, ou queda entre bloquear e gravar o id: adota só o bloqueio com o motivo deste destino
                        if str((json.loads(last[1] or '{}') or {}).get('reason') or '').startswith(f"Destino {wait.get('target')} "):
                ...
                if escalation and escalation['status'] == 'resolved' and escalation['action'] == 'changes':
                    with _kb().write_txn(conn, allow_nested=True):  # revisão 4: endereço novo ou correção do Principal; o card volta ao worker
                        released = _kb().unblock_task(conn, task_id)
                        if released:
                            _store_destination_wait(conn, task_id, None)
                            _destination_next_action(conn, task_id, f"Principal (changes, {escalation['id']}; íntegra no contexto do caso): "  # revisões 6 e 7
                                                     f"{escalation['answer'] or ''}")
                ...
                if now < next_check or budget <= 0:
                    if _json(wait) != original:  # grava só quando a espera mudou (adoção), não a cada tick
                        _store_destination_wait(conn, task_id, wait)
                    continue
                budget -= 1
                ...
                if up and wait.get('up_at') and now - int(wait['up_at']) >= DESTINATION_CONFIRM_SECONDS:
                    with _kb().write_txn(conn, allow_nested=True):  # revisão 4: desbloqueio, fim da espera e retirada da escalada juntos
                        back = _kb().unblock_task(conn, task_id)
                        if back:
            ...
            if conn.execute("SELECT 1 FROM nfos_decisions WHERE task_id=? AND status IN ('pending','human') LIMIT 1", (task_id,)).fetchone():
                continue
            candidate = _destination_wait_candidate(conn, task_id)
            if not candidate:
                continue
            target, criterion, error, since = candidate
            budget -= 1
            up = _destination_reachable(target)
            check = {'checked_at': now, 'up': up, 'next_check_at': now + DESTINATION_RECHECK_SECONDS}
            if up:
                with _kb().write_txn(conn, allow_nested=True):
                    state = json.loads(get_workflow(conn, task_id)['state_json'] or '{}') or {}
                    state['destination_check'] = check
                    _save_workflow_state(conn, task_id, state)
                continue
            ...
            with _kb().write_txn(conn, allow_nested=True):  # revisão 4: a espera nasce antes do bloqueio; queda entre os dois é adotada pelo alvo
                ...
                _event(conn, task_id, None, 'nfos_destination_wait', {'target': target, 'criterion': criterion, 'error': error[:200], 'since': since,
                                                                       'wait_id': wait['token']})  # revisão 7: id da espera
    ...
    try:
        ...
        for entry in sorted(vault.iterdir()):
            ...
            for p in paths[:40]:
                ...
                files.append(item)
        if not files:
            return {'vault':str(vault),'files':[],'note':'Nenhum arquivo deste projeto no cofre. Credencial que você consegue obter (login legítimo, acesso que já existe no destino) vai para o cofre da sonda com `probe-env --set NOME`; pergunte ao dono só a informação que não existe em nenhum acesso seu, nunca sobre sonda ou runtime.'}  # HUMAN_LAST_RESORT_20260914
        return {'vault':str(vault),'files':files,
                'how':'Carregue um .env dentro do seu comando: set -a; . <arquivo>; set +a. Arquivos .json são estados de sessão/acessos: leia e use. Nunca cole valores em cards, relatórios, commits ou chat. Credencial de sonda: `probe-env --set NOME` (valor pela entrada padrão), lida pela sonda como $env:NOME.'}  # HUMAN_LAST_RESORT_20260914
    except Exception:
        return None
...
def main():
    ...
    parser.add_argument('--set',dest='set_name',help='probe-env: variable name; the value comes from standard input')  # HUMAN_LAST_RESORT_20260914
    parser.add_argument('--unset',dest='unset_name',help='probe-env: variable name to remove')  # HUMAN_LAST_RESORT_20260914
    ...
    with _kb().connect_closing(db_path=Path(args.db) if args.db else None) as conn:
        ...
        elif args.action=='probe-env':  # HUMAN_LAST_RESORT_20260914: valor só pela entrada padrão, nunca em argumento
            result=probe_env_command(conn,args.task,set_name=args.set_name,unset_name=args.unset_name,
                                     value=sys.stdin.read() if args.set_name else None)
