            ...
            try:  # BLOCK_LESS3_20260910: premissas do owner na primeira chamada de todo worker
                ...
                if not _req(conn,args.task):
                    result['owner_premises']=[
                        ...
                        '6. Code card with size P (small fix): no mandatory failing test first (add tests only when cheap), no staging or HML homologation. Set delivery_destination environment=pr (review PR with CI on the exact candidate; target = the repository URL), branch from main, open the PR, wait for green CI, ask kind=review once; on approve merge the PR yourself (gh pr merge --merge), wait for the automatic deploy, read production back, save the report with that readback and call kanban_complete. Sizes M and G keep the staging homologation.',  # SMALL_CODE_FAST_20260910
