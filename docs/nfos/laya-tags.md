...
O serviço `scripts/nfos_laya_server.py` usa o SDK oficial em revisão fixa e safetensors locais, sem código remoto confiado ou checkpoint inglês. Exige Transformers5.0.0: a versão4.57 interpreta incorretamente o parâmetro local RoPE do checkpoint. O checkpoint tem max_len=1024 e head_max_len=256; o encoder declara 8192 posições, mas esse limite maior não é promovido a capacidade validada. O serviço verifica tokens reais de estado, instruções e opções antes da chamada. Qualquer corte que o SDK faria retorna HTTP422 context_too_large. Não há truncamento silencioso com aceite.
