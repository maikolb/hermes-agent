...
- The portal currently imports `HermesCLI`, invokes `_init_agent`, and assigns internal callbacks in `hermes_bridge.py`; `hermes serve` exposes session creation but no stable remote turn/redirect/event contract for this client.

## Root-Cause Hypothesis
- Facts: active-turn redirect and normalized internal callbacks already exist; the public server currently uses PTY/dashboard-specific paths for chat execution.
- Assumptions: a backward-compatible JSON-RPC extension can delegate to existing primitives without changing CLI behavior.
...
- Completion: validated-local; target VPS/provider execution not run
