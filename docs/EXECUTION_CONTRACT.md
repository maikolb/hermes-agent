...
```json
{
  ...
  "checks": [
    ...
    {
      "criterionId":"AC-010","status":"failed","performedBy":"agent","verificationMode":"direct","method":"Focused tests, adjacent tests, Ruff, compile, diff, AOF and required CI","target":"declared changed files and PR #79","procedure":"Ran all local gates, pushed correction series, then waited for required Python and Nix jobs.","expected":"All local and required remote gates green.","observed":"Local gates are green: 25 focused and 37 adjacent tests, Ruff, py_compile and diff check. Python CI failed 78 tests across 47 files; comparable accepted-base runs also fail broadly. Nix remained queued without a result.","performedAtUtc":"2026-09-04T01:16:19Z","artifacts":[{"path":"internal/ops/evidence/P0-4A_LOCAL_VALIDATION.md","sha256":"80384e9b1bdadab699e379be7afa0318531434a12ce6de410bf7fa638ef9a9d7"}]
    },
    {
...
- Validation: local code and behavior gates pass. Required Python CI failed and Nix is pending, so current readiness remains `implemented`.
- Completion: blocked-with-evidence from `validated-local` by failed required Python CI and unresolved queued Nix CI.
