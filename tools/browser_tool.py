    ...
    if cdp_override and not force_local:
        ...
        if provider is None:
            ...
            try:
                ...
                logger.warning(
                    "Cloud provider %s failed (%s); refusing implicit local "
                    "Chromium fallback for task %s",
                    provider_name, e, task_id,
                    exc_info=True,
                )
                raise RuntimeError(
                    f"Cloud provider {provider_name} failed; implicit local "
                    f"browser fallback is disabled ({e})"
                ) from e
